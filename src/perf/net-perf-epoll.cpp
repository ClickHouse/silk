...
int Event::create(Event * out) noexcept
{
    ...
    {
        int r = errno;
        LOG_ERROR("eventfd failed: {}", std::strerror(r));
        return r;
    }
    *out = Event(fd);
    return 0;
...
int Event::trigger() noexcept
{
    ...
    {
        int r = errno;
        LOG_ERROR("write eventfd failed: {}", std::strerror(r));
        return r;
    }
    return 0;
}
...
int Epoll::create(Epoll * out) noexcept
{
    ...
    {
        int r = errno;
        LOG_ERROR("epoll_create1 failed: {}", std::strerror(r));
        return r;
    }
    *out = Epoll(fd);
    return 0;
...
int Epoll::add(int fd, uint32_t events, void * ptr) noexcept
{
    ...
    {
        r = errno;
        LOG_ERROR("epoll_ctl ADD failed: {}", std::strerror(r));
        return r;
    }
    return 0;
}
...
int Epoll::del(int fd) noexcept
{
    ...
    {
        r = errno;
        LOG_ERROR("epoll_ctl DEL failed: {}", std::strerror(r));
        return r;
    }
    return 0;
}
...
int Epoll::wait(epoll_event * events, int maxEvents, int timeoutMs, int * outCount) noexcept
{
    ...
    if (n < 0)
    {
        ...
        if (r != EINTR)
        {
            LOG_ERROR("epoll_wait failed: {}", std::strerror(r));
        }
        return r;
    }
    *outCount = n;
...
int TcpConnection::connect(const char * host, uint16_t port, TcpConnection * out) noexcept
{
    ...
    {
        r = errno;
        LOG_ERROR("fcntl F_GETFL failed: {}", std::strerror(r));
        ::close(fd);
        return r;
    }
    ...
    {
        r = errno;
        LOG_ERROR("fcntl F_SETFL O_NONBLOCK failed: {}", std::strerror(r));
        ::close(fd);
        return r;
    }
...
int TcpConnection::listen(const char * host, uint16_t port, int backlog, TcpConnection * out) noexcept
{
    ...
    {
        r = errno;
        LOG_ERROR("setsockopt SO_REUSEPORT failed: {}", std::strerror(r));
        ::close(fd);
        return r;
    }
...
void Server::start()
{
    ...
    for (Worker & worker : workers)
    {
        ...
        ASSERT(!r, "Event::create failed: {}", std::strerror(r));
...
bool Server::drive(Connection * conn, uint32_t msgSize) noexcept
{
    for (;;)
    {
        if (conn->state == State::Reading)
        {
            ...
            {
                LOG_ERROR("read failed: {}", std::strerror(r));
            }
            return false;
        }
        ...
        if (!isExpectedShutdown(r))
        {
            LOG_ERROR("write failed: {}", std::strerror(r));
        }
        return false;
    }
}
...
void Server::workerMain(WorkerParams params) noexcept
{
    ...
    ASSERT(!r, "Epoll::create failed: {}", std::strerror(r));
    ...
    while (!stop)
    {
        ...
        ASSERT(!r, "epoll_wait failed: {}", std::strerror(r));
            ...
            if (tag == EventTag::Listener)
            {
                for (;;)
                {
                    ...
                    if (ar)
                    {
                        ...
                        {
                            LOG_ERROR("accept failed: {}", std::strerror(ar));
                        }
                        break;
                    }
...
static void runServer(int argc, char ** argv)
{
    ...
    LOG_INFO("starting server on {}:{} with {} thread(s)", cfg.host, cfg.port, cfg.threads);
...
static void runClient(int argc, char ** argv)
{
    ...
    LOG_INFO("starting client on {}:{} with {} thread(s) and {} connection(s)", cfg.host, cfg.port, cfg.threads, cfg.numConnections);
