...
{
public:
    Profiler(uint32_t targetTgid, uint32_t sampleHz, bool kernelStacks, bool offcpu) noexcept;
    ~Profiler() noexcept;
    ...
    void collect(Symbolizer * symbolizer);

private:
    static constexpr int MAX_FRAMES = 127;

    using StackMap = std::unordered_map<uint64_t, std::pair<uint64_t, uint64_t>>;
    static void drainMap(int fd, StackMap & merged, bool isOnCpu, uint64_t weightFactor);
    static int countFrames(const uint64_t * addrs);
