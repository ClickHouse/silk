...
A non-fiber OS thread (the application's `main`, a thread-pool worker, any external thread) can still participate in every fiber-aware API. The first time such a thread calls `getCurrentFiber`, the scheduler hands it a **proxy fiber** (`isProxyFiber = true`) that represents the thread itself, held in a `thread_local` slot; it owns no stack. At thread exit the slot returns the proxy to the scheduler's `proxyFiberPool` and the next new thread reuses it; `destroy` frees the pool together with `fiberPool`. While the scheduler is initialized a proxy is never deleted: sync primitives keep raw `Fiber *` snapshots in their state (the `FiberMutex` owner, for example) and may read one after the owning thread has exited, so proxy memory stays valid exactly as `fiberPool` memory does for regular fibers. A thread that exits with no scheduler initialized, before `initialize` or after `destroy`, deletes its own proxy: no fiber exists that could hold a stale pointer to it. The slot chooses between the pool and `delete` by reading the scheduler pointer that `destroy` clears, so a thread that has used a fiber-aware API exits either before `destroy` or after it, never concurrently with it.
